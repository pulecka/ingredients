...
$templateCache.put("app/modules/iForm/components/error/error.template.html","<i-alert ng-if=\"show\" type=\"error\" messages=\"errors\"></i-alert>");
$templateCache.put("app/modules/iForm/components/row/row.template.html","<div class=\"i-row {{ classes }}\"><div ng-if=\"rowLabel\" class=\"i-row-label\" ng-bind=\"rowLabel\"></div><div class=\"i-row-content\" ng-class=\"{labeled: rowLabel}\" ng-transclude=\"\"></div></div>");
$templateCache.put("app/modules/iForm/components/select/list.template.html","<div class=\"i-select-list\" ng-show=\"vm.listToggle\"><div class=\"is-array\" ng-if=\"isArray\"><div ng-if=\"vm.template\" ng-include=\"vm.template\" class=\"i-select-item\" ng-repeat=\"item in data | filter: vm.searchQuery track by $index\" ng-click=\"vm.select($index)\"></div><div ng-if=\"!vm.template\" class=\"i-select-item\" ng-repeat=\"item in data | filter: vm.searchQuery track by $index\" ng-click=\"vm.select($index)\" ng-bind=\"item[viewAs]\"></div></div><div class=\"is-object\" ng-if=\"isObject\"><div ng-if=\"vm.template\"><div ng-include=\"vm.template\" class=\"i-select-item\" ng-repeat=\"(key, item) in data\" ng-click=\"vm.select(key)\"></div></div><div ng-if=\"!vm.template\"><div class=\"i-select-item\" ng-repeat=\"(key, item) in data\" ng-click=\"vm.select(key)\">{{item[viewAs]}}</div></div></div></div>");
$templateCache.put("app/modules/iForm/components/select/select.template.html","<div class=\"i-select\" style=\"width:{{size || 100}}%\" ng-if=\"!native\"><div ng-hide=\"searchable\" class=\"current-value\" ng-click=\"iSelect.listToggle=!iSelect.listToggle\"><span>{{iSelect.selected[viewAs]}}</span></div><div ng-show=\"searchable\" class=\"i-select-search\"><input type=\"text\" ng-model=\"iSelect.searchQuery\" ng-change=\"iSelect.listToggle = true\" placeholder=\"{{placeholder}}\"></div><div class=\"i-select-toggle\" ng-click=\"iSelect.listToggle=!iSelect.listToggle\"><div class=\"ion-arrow-down-b i-select-toggle-icon\"></div></div><div class=\"i-select-list\" ng-show=\"iSelect.listToggle\"><div class=\"is-array\" ng-if=\"isArray\"><div ng-if=\"template\" ng-include=\"template\" class=\"i-select-item\" ng-repeat=\"item in data | filter: iSelect.searchQuery track by $index\" ng-click=\"iSelect.select($index)\"></div><div ng-if=\"!template\" class=\"i-select-item\" ng-repeat=\"item in data | filter: iSelect.searchQuery track by $index\" ng-click=\"iSelect.select($index)\" ng-bind=\"item[viewAs]\"></div></div><div class=\"is-object\" ng-if=\"isObject\"><div ng-if=\"template\"><div ng-include=\"template\" class=\"i-select-item\" ng-repeat=\"(key, item) in data\" ng-click=\"iSelect.select(key)\"></div></div><div ng-if=\"!template\"><div class=\"i-select-item\" ng-repeat=\"(key, item) in data\" ng-click=\"iSelect.select(key)\">{{item[viewAs]}}</div></div></div></div></div>");
$templateCache.put("app/modules/iForm/components/switch/iSwitch.template.html","<div ng-init=\"vm.model = vm.model || vm.options[vm.default][vm.returnAs] || vm.options[vm.default] || null\" class=\"i-switch\" style=\"width:{{ (vm.size || 100) }}%\"><div ng-repeat=\"option in vm.options track by $index\" class=\"i-switch-option\" ng-bind=\"option[vm.viewAs] || option || $index\" ng-click=\"vm.handleClick(option, $index)\" ng-class=\"{active: vm.model == (option[vm.returnAs] || $index)}\" style=\"width: {{100/vm.options.length}}%\"></div></div>");
