...
$templateCache.put("app/modules/iForm/components/button/button.template.html","<button ng-class=\"{fill: fill, small: small}\" class=\"i-button\" ng-disabled=\"disabled\" name=\"{{name}}\" type=\"{{type || \'button\'}}\">{{title}}</button>");
$templateCache.put("app/modules/iForm/components/alert/alert.template.html","<div class=\"i-alert i-alert-{{ alert.type }}\"><div ng-if=\"alert.label\" class=\"i-alert-header\" ng-bind=\"alert.label\"></div><div ng-repeat=\"error in alert.list\">{{error.name}}: {{error.error}}</div></div>");}]);